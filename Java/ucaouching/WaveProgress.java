...
import com.ambiq.ble.ambiqota.utils.Constant;
import com.ambiq.ble.ambiqota.utils.MiscUtil;
...
public class WaveProgress extends View {
    ...
    public WaveProgress(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
        new Thread(new HumidityUpdateThread()).start();
    }
...
//        registerBroadcastReceiver();
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        mValue = typedArray.getFloat(R.styleable.WaveProgress_value, /*Constant.DEFAULT_HUMIDITY_VALUE*/Constant.DEFAULT_VALUE);
...
//            Log.d(TAG, "mPercent = " + mPercent + "; mPrePercent = " + mPrePercent);
        ...
        if (mPrePercent == 0.0f || Math.abs(mPercent - mPrePercent) >= 0.001f) {
            mPercentValue = String.format("%.1f%%", mPercent * 100);
    ...
    private void startAnimator(final float start, float end, long animTime) {
        ...
        mProgressAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                if (mPercent == 0.0f || mPercent == 0.5f) {
                    stopWaveAnimator();
                } else {
                    startWaveAnimator();
                }
    ...
    class HumidityUpdateThread implements Runnable {
        public void run() {
            while (!Thread.currentThread().isInterrupted()) {
                ...
                //scrollBy(1,0);
                if (TestShowViewActivity.humidity != 0.0f) {
                    mPercent = TestShowViewActivity.humidity/100.0f;
...
//        Humidity_Server_Broadcast receiver = new Humidity_Server_Broadcast();
//        IntentFilter filter = new IntentFilter(TestShowViewActivity.RecvDataStatus);
//        mContext.registerReceiver(receiver, filter);
//    }
//
//    public class Humidity_Server_Broadcast extends BroadcastReceiver {
//        @Override
//        public void onReceive(Context context, Intent intent) {
//            String action = intent.getAction();
//            Float tmp_f;
//            Log.d(TAG, "Server_Broadcast WaveProgress get data");
//            if (action.equals(TestShowViewActivity.RecvDataStatus)) {
//                Bundle bundle = intent.getExtras();
//                String str = bundle.getString("view_data");
//                Log.d(TAG, "get sensor data:" + str);
//                if (str.equals("HT")) {
////                    humidityView.setText(TestBLEActivity.Humidity);
//                    //tempView.setText(TestBLEActivity.Temp);
//                    setValue(TestShowViewActivity.humidity);
////                    setValue(TestShowViewActivity.humidity);
//                }
//            }
//        }
