...
public class CircleProgress extends View {
    ...
    public CircleProgress(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
        new Thread(new UVUpdateThread()).start();
    }
...
//        registerBroadcastReceiver();
    ...
    private void initAttrs(AttributeSet attrs) {
        ...
        mValue = typedArray.getFloat(R.styleable.CircleProgressBar_value, Constant.DEFAULT_UV_LEVEL/*Constant.DEFAULT_VALUE*/);
    ...
    class UVUpdateThread implements Runnable {
        public void run() {
            while (!Thread.currentThread().isInterrupted()) {
                ...
                //scrollBy(1,0);
                if (TestShowViewActivity.uvlevel != 0.0f) {
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            setValue(TestShowViewActivity.uvlevel);
                        }
                    });
//                    mValue = TestShowViewActivity.temprature;
//                    postInvalidate();
                }
//                mValue = TestShowViewActivity.temprature;
//                if (mValue !=  0.0f) {
...
//        UV_Server_Broadcast receiver = new UV_Server_Broadcast();
//        IntentFilter filter = new IntentFilter(TestShowViewActivity.RecvDataStatus);
//        mContext.registerReceiver(receiver, filter);
//    }
//
//    public class UV_Server_Broadcast extends BroadcastReceiver {
//        @Override
//        public void onReceive(Context context, Intent intent) {
//            String action = intent.getAction();
//            Float tmp_f;
//            Log.d(TAG, "Server_Broadcast CircleProgress get data");
